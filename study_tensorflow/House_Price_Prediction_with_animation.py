...
# generation some house sizes between 1000 and 3500 (typcial sq ft of house)
num_house=160
np.random.seed(42)
house_size=np.random.randint(low=1000,high=3500,size=num_house)

# Generation house_price from house size with a random noise added
np.random.seed(42)
...
#need to normalize values to prevent /overflows
def normalize(array):
...
# Notice, the use of tensorflow add an mulitply functions. These add the operations to the computation graph,
...
#4. Define a Gradient decent optimizer that will minimize the loss defined in the operation "cost".
...
with tf.Session() as sess:
    ...
    #add storage of factor and offset valuses from each epoch
    ...
    for iteration in range(num_train_iter):
        ...
        if (iteration+1)%display_every==0:
            ...
            # Save the fit size and price_offset to allow animation fo learning process
    ...
    # find the values that returned the "best" fit lines
    fig,ax=plt.subplot()
    ...
    plt.ylabel("Price")
    plt.xlabel("Size(sq.ft)")
    plt.plot(train_house_size,train_price,'go',lable='Training data')
    ...
    # Inint only required for blitting to give a chean slate
    def initAnim():
        line.set_ydata(np.zeros(shape=house_price.shape[0]))   #set y's to 0
        return line,

    ani=animation.FuncAnimation(fig,animate,frames=np.arrange(0,fit_plot_idx),init_func=initAnim,
